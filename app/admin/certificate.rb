ActiveAdmin.register Certificate do
  menu priority: 3, label: proc { I18n.t 'active_admin.certificates' }

  permit_params :active,
                :number, :object, :classifier, :classifier_code, :grouping,
                :issued_at, :closed_at,
  ...
  index as: :table_alter do
    ...
    column :number, class: 'h-size--10'
    column :classifier
    column 'Код', :classifier_code
    column 'Группировка', :grouping
    column :issued_at, class: 'h-size--datetime'
    column :closed_at, class: 'h-size--datetime'
    actions
  end
    ...
    tabs do
      tab 'Данные' do
        f.inputs do
          ...
          f.input :classifier, input_html: {class: 'h-size--20'}
          f.input :classifier_code, input_html: {class: 'h-size--20'}
          f.input :grouping, input_html: {class: 'h-size--20'}
          ...
          f.input :agent, :as => :select, include_blank: true,
                  collection: Agent.certification.collect { |model| [model.name, model.id] },
                  input_html: {class: 'custom-select size-40'}
        end
      end
      tab 'Этапы' do
        f.inputs do
          f.has_many :stages, heading: false, allow_destroy: true, new_record: 'Добавить' do |n|
            n.input :operate, as: :select, include_blank: false,
                        collection: Stage::OPTIONS.to_a,
                        input_html: {class: 'custom-select'}
            n.input :operate_at, as: :date_picker
            n.input :operation
            n.input :passed_at, as: :date_picker
          end
        end
  ...
  action_item :inspection_certificates, only: :index do
  ...
  collection_action :inspection, method: :patch do
    ...
    redirect_back fallback_location: admin_dashboard_path, notice: "[#{done}] Инспекция сертификатов проведена!"
  end
end
